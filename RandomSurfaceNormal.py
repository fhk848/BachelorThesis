...
Us = np.arange(0.0, 1.1, step = 0.01)

# Get random array of ∆G_i for a surface of 100x100 atoms 
# chosen by an normal distrubution with mean = 0 and standard deviation = 0.25
G_i = np.random.normal(loc = 0, scale = 0.25, size = (l_surface, l_surface))
...
plt.title(f'Polarization Curves for a Random HEA Surface (Normal Distribution)')
plt.legend()
